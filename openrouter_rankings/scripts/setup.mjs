 ...
 * and generate src/snapshot.json (Remotion) plus public/data.js (legacy).
 *
 * Runs automatically in CI before validate/render; falls back to the
 * committed Remotion snapshot when the fetch fails; fails if none exists.
 *
...
const SNAPSHOT_FILE = path.join(ROOT, "src", "snapshot.json");
...
async function main() {
  ...
  try {
    ...
    console.warn(`⚠ fetch failed (${err.message}) — keeping committed snapshot ${SNAPSHOT_FILE}`);
    if (!fs.existsSync(SNAPSHOT_FILE)) {
      throw new Error("No Remotion snapshot available and fetch failed", {cause: err});
    }
    return;
  }
  // Surface write errors instead of reporting a successful fallback after a partial update.
  writeSnapshot(data);
}
