...
基于 **Remotion**（React）构建，旁白使用 **Edge TTS**（微软神经语音），地图使用 **OpenStreetMap 真实地理数据**。
...
| Línea 1 地图 | 真实地图 + L1 线路动画绘制：34.6 km、26 站、高架线 | 10.0–18.8s |
| Línea 1 纪录 | 2011 年开通、2014 年全线；拉丁美洲最长高架地铁 | 18.8–25.4s |
| Línea 2 地图 | 真实地图 + L2 线路（运营段 + 在建段）：27 km、27 站、无人驾驶 | 25.4–33.7s |
...
- 线路：Línea 1（relation 7727213/1408101）、Línea 2 运营段（16861881/16861884）、Línea 2 在建段（railway=construction）
- 车站：34 个 `station=subway` 节点；海岸线：`natural=coastline`
- 投影：d3-geo `geoMercator` + `fitExtent`（竖屏画布）
- 原始 Overpass 数据在 `data/`，经 `scripts/convert_osm.py`、`scripts/assemble_l2.py` 转为 `public/data/*.geojson`
...
metro-lima/
  src/                      # Remotion composition（TSX）
    ...
    Map.tsx                 # d3-geo 地图渲染组件
    timing.ts               # 场景时间轴与动画工具
  scripts/                  # 数据与音频生成脚本
    convert_osm.py          # Overpass JSON → GeoJSON
    assemble_l2.py          # L2 在建段组装
    gen_voiceover.py        # Edge TTS 旁白生成 + 拼接
  data/                     # Overpass 原始数据（不入库）
  public/
    data/*.geojson          # 地图数据（入库）
    voiceover/              # 旁白文本与音频
