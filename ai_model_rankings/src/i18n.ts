// Port of public/i18n.js. {{placeholders}} are resolved in AiModelRankings.tsx
// from the static data.ts snapshot — never at render time via fetch.
export type LocaleKey = 'en' | 'zh' | 'ja';

export type MetricCopy = {label: string; value: string; color: string};
export type EffCardCopy = {label: string; title: string; value: string; color: string};
export type TierRowCopy = {label: string; note: string; color: string};

export type Locale = {
  fontFamily: string;
  providers: Record<string, string>;
  scenes: {
    intro: {kicker: string; title: string; subtitle: string; metrics: MetricCopy[]};
    ranking: {kicker: string; title: string; subtitle: string};
    scatter: {kicker: string; title: string; subtitle: string; xAxis: string; yAxis: string};
    efficiency: {kicker: string; title: string; subtitle: string; cards: EffCardCopy[]};
    tiers: {kicker: string; title: string; subtitle: string; rows: TierRowCopy[]};
    providers: {kicker: string; title: string; subtitle: string};
    outro: {kicker: string; title: string; bullets: string[]};
  };
};
...
const en: Locale = {
  ...
  providers: {
    ...
    zai: 'Z AI', alibaba: 'Alibaba', minimax: 'MiniMax', kimi: 'Kimi', xiaomi: 'Xiaomi',
    nexagi: 'Nex AGI', xai: 'xAI', stepfun: 'StepFun', inception: 'Inception', others: 'Others',
  },
  scenes: {
    intro: {
    ...
    outro: {
      ...
      bullets: ['{{total}} models evaluated', '9 independent benchmarks', 'Data from Artificial Analysis'],
    },
  },
};
...
const zh: Locale = {
  ...
  providers: {
    anthropic: 'Anthropic', openai: 'OpenAI', google: 'Google', deepseek: '深度求索',
    zai: 'Z AI', alibaba: '阿里云', minimax: 'MiniMax', kimi: '月之暗面', xiaomi: '小米',
    nexagi: 'Nex AGI', xai: 'xAI', stepfun: '阶跃星辰', inception: 'Inception', others: '其他',
  },
  scenes: {
    intro: {
      kicker: '2026 AI 模型排名',
    ...
    outro: {
      ...
      bullets: ['评估 {{total}} 款模型', '9 项独立基准测试', '数据来源：Artificial Analysis'],
    },
  },
};
...
const ja: Locale = {
  ...
  providers: {
    anthropic: 'Anthropic', openai: 'OpenAI', google: 'Google', deepseek: 'DeepSeek',
    zai: 'Z AI', alibaba: 'Alibaba', minimax: 'MiniMax', kimi: 'Kimi', xiaomi: 'Xiaomi',
    nexagi: 'Nex AGI', xai: 'xAI', stepfun: 'StepFun', inception: 'Inception', others: 'その他',
  },
  scenes: {
    intro: {
    ...
    outro: {
      ...
      bullets: ['{{total}} モデルを評価', '9つの独立ベンチマーク', 'データ出典：Artificial Analysis'],
    },
  },
};

export const LOCALES: Record<LocaleKey, Locale> = {en, zh, ja};
