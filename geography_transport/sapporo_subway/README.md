...
`public/voiceover/narration.ja.mp3` 和 `segment-durations.json` 是生成产物，不提交到 Git。GitHub Actions 会在类型检查和正式渲染前自动安装 `edge-tts` 并生成这两个文件；本地首次运行或修改旁白后，也需要先执行 `npm run voiceover`。
...
```text
geography_transport/sapporo_subway/
