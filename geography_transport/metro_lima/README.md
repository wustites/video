...
`public/voiceover/narration.es.mp3` 和 `segment-durations.json` 是生成产物，不提交到 Git。GitHub Actions 会在类型检查和正式渲染前自动生成；本地首次运行或修改旁白后，也需要先执行 `npm run voiceover`。
...
```text
geography_transport/metro_lima/
